...
1.  [Set the download directory](#downloading-data), i.e., the folder on
    ...
    This is now referred to as the “prism archive”.
...
Important .bil files are no longer available in .bil format. The
...
This function gives a list of folder names, i.e., prism data (`pd`). All
the functions in the prism package work off of one or more of these
folder names (`pd`).
...
absolute path (e.g. the `raster` package). The
[`pd_to_file()`](https://docs.ropensci.org/prism/reference/pd_get.md)
function conveniently returns the absolute path. Alternatively, you may
want to see what the normal name for the product is (not the file name),
and we can get that with the
...
is a convenient way to search for specific parameters, time steps, days,
months, years, or ranges of days, months, years. **Note that resolution
must be specified for all archive subset operations.**
...
![](reference/figures/README-quick_plot-1.png)
...
![](reference/figures/README-raster_math-1.png)
...
This procedure will take a set of rasters, create a
“[`raster::stack`](https://rdrr.io/pkg/raster/man/stack.html)”, extract
data at a point, and then create a ggplot2 object.
...
[`pd_plot_slice()`](https://docs.ropensci.org/prism/reference/pd_plot_slice.md)
returns a gg object, it can be combined with other ggplot functions.
...
![](reference/figures/README-plot_Boulder-1.png)
...
\[![leaflet example figure](articles/leaflet_example.png)
